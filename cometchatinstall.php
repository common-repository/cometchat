...
EOD;
	...
	if(empty($ccintialaccess) && is_plugin_active('buddypress/bp-loader.php')){
	...
	if(is_dir($cometchat_dir) && file_exists($cometchat_dir. DIRECTORY_SEPARATOR .'license.php') && is_dir($cometchat_dir. DIRECTORY_SEPARATOR .'admin') || !empty($cc_clientid) || !empty($_COOKIE['cc_cloud'])){
		...
		if(empty($ccintialaccess)){
			$adminpaneliframe = <<<EOD
				...
				<div class="cometchat_outerframe" id="cc_outerframe" style="opacity:0.1">
					<div class="cometchat_middleform" >
						<div class="cometchat-tab-content" onclick="">
				            <div id="cometchat_adminpanel">
				            	<div class="cometchat_logo_div" style="margin-bottom: 0px !important;" id="ccimg">
								...
								<div id="ccinitialaccess" style="background: #ffffff;width: 50%;margin: 0px auto;border-radius:5px;">
									...
									<p style="color: black !important;">
										...
										<button type="button" id="cc_plugin_admin_panel" class="ccadminpanel ccadminpanel-primary" onclick="openCCAdminPanel('{$adminpanelurl}');" style="margin-bottom: 10px;">Launch Admin Panel</button>
									$selfhostedcc
								</div>
				            </div>
			        	</div>
...
?>
		<div id="cc_dom_ready"><?php echo $adminpaneliframe; ?></div>
